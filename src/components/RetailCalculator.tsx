  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-slate-900">Retail Pricing Suite</h2>
        <p className="text-slate-500">Calculate profitable retail prices with all hidden costs accounted for</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-4">
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full p-3 pl-8 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none"
                    />
                  </div>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">$</span>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Labor Rate ($/hr)</label>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">$</span>
                        <input 
                          type="number"
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Labor Hours</label>
                      <div className="relative">
                        <input 
                          type="number"
                          value={laborHours || ''}
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">CAPEX / Mold Fees</label>
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">$</span>
                      <input 
                        type="number"
          ...
          <div className="bg-emerald-50 p-8 rounded-3xl border border-emerald-100 space-y-6">
              ...
              <div className="flex bg-white p-1 rounded-xl border border-emerald-200">
                <button 
                  ...
                  className={cn(
                    ...
                    isMarkup ? "bg-emerald-600 text-white shadow-sm" : "text-emerald-600 hover:bg-emerald-50"
                  )}
                >
                  MARKUP
                </button>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-black text-emerald-900">{markupValue}%</span>
              </div>
              <div className="relative pt-2">
                ...
                <div className="relative w-full mt-4 h-6">
                  <span className="absolute left-0 text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Low</span>
                  <button 
                  ...
                  >
                    <div className="w-0.5 h-2 bg-emerald-400 mb-1" />
                    <span className="text-[10px] font-bold text-emerald-700 uppercase tracking-tight whitespace-nowrap bg-white px-1.5 py-0.5 rounded border border-emerald-100 shadow-sm group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                      {isMarkup ? 'Keystone (100%)' : 'Standard (50%)'}
                    </span>
                  </button>
                  <span className="absolute right-0 text-[10px] font-bold text-emerald-600 uppercase tracking-widest">High</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 space-y-4">
            <h4 className="font-bold text-slate-900 text-sm">Price Breakdown</h4>
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-amber-400" />
                <span className="text-xs text-slate-500 flex-1">Materials</span>
                <span className="text-xs font-bold text-slate-900">${(metalCost + stoneCost).toFixed(2)}</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-blue-400" />
                <span className="text-xs text-slate-500 flex-1">Labor & Ops</span>
                <span className="text-xs font-bold text-slate-900">${(laborCost + capex).toFixed(2)}</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full bg-indigo-400" />
                <span className="text-xs text-slate-500 flex-1">Overhead & Ship</span>
                <span className="text-xs font-bold text-slate-900">${(shipping + (baseCost * overhead / 100)).toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
